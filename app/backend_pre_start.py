...
settings = Settings()
ASYNC_URL: URL = settings.db.SQLALCHEMY_DATABASE_URI
...
async def _ping_db_once() -> None:
    ...
    if ASYNC_URL.host and ASYNC_URL.port and not _tcp_check(ASYNC_URL.host, int(ASYNC_URL.port)):
        raise ConnectionError(f"No hay conexion a {ASYNC_URL.host}:{ASYNC_URL.port}")
    ...
    try:
        async with engine.connect() as conn:
            ...
            db, user, host, port, search_path = res.fetchone()
            logger.info(
