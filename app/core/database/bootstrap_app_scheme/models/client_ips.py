...
class ClientIPs(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("integration_client_cod", "cidr", name="uq_client_ip_per_client"),
        {"schema": schema_name},
    )

    codClientIp: int | None = Field(default=None, sa_column=Column("cod_client_ip", Integer, primary_key=True, autoincrement=True))
    ...
    integrationClient: ClassVar["IntegrationClients"] = relationship(
        "IntegrationClients", back_populates="ips", lazy="selectin"
    )
