...
from sqlalchemy import Integer, String, Boolean, UniqueConstraint, text
...
class IdempotencyKeys(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("client_id", "key", name="uq_client_idem"),
        {"schema": schema_name},
    )

    codIdempotencyKeys: Optional[int] = Field(
        default=None,
        sa_column=Column("cod_idempotency_keys", Integer, primary_key=True, autoincrement=True),
    )
    createDate: datetime = Field(
        sa_column=Column("create_date", TIMESTAMP(timezone=True), nullable=False, server_default=func.now()),
    )
    updateDate: Optional[datetime] = Field(
        default=None,
        sa_column=Column("update_date", TIMESTAMP(timezone=True), onupdate=func.now()),
    )
    deleteDate: Optional[datetime] = Field(
        default=None,
        sa_column=Column("delete_date", TIMESTAMP(timezone=True)),
    )
